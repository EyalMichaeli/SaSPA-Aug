...
def main(base_model, controlnet, SDEdit):
    try:
        ...
        if not PROMPT_TYPE == "captions": 
            with open(str(prompts_file), "r") as f:
                prompts = f.readlines()
...
if __name__ == "__main__":
    """
    CUDA_VISIBLE_DEVICES=0 nohup sleep 10m; python run_aug/run_aug_real_guidance.py > run_aug/run_aug_real_guidance.log 2>&1 &
    CUDA_VISIBLE_DEVICES=0 nohup python run_aug/run_aug_real_guidance.py > run_aug/run_aug_real_guidance.log 2>&1 & 
    CUDA_VISIBLE_DEVICES=0 python run_aug_real_guidance/run_aug_real_guidance.py
    """
    ...
    CONTROLNET = None  # "canny"  # out of None, "canny", "hed"
    SDEDIT = 1  # if True, will do IMG2IMG edit. LECF is from scratch, ALIA is 0.5
    NUM_PER_IMAGE = 2 # number of augmentations to generate per image
    SEED = 1
    
    # prompts params:
    PROMPT_TYPE = "txt2sentence"  # out of "txt2sentence", "txt2sentence-per_class", "captions", "gpt-meta_class", "ALIA"
    PROMPT_WITH_SUB_CLASS = True  # the prompt will contain the sub class of the image (e.g. "a Boing 707-320 plane ...")
    USE_ARTISTIC_PROMPTS = False if BASE_MODEL == "sd_v1.5" else False  # feel free to change this. if True, will use artistic prompts (e.g. "a painting of a ...") at the end of used prompts
    ...
    NUM_INFERENCE_STEPS = 50  # 20-50 is usually used. for SD XL turbo, 1-4 steps are used
    # SDEdit (img2img) params
    SDEDIT_STRENGTH = 0.15  # range is 0-1. relevant only if SDEdit is true. for ALIA: 0.5. For RG(LECF): 0.15.
    ...
    LPIPS_MIN = None
    LPIPS_MAX = None
    CLIP_FILTERING_TYPE = "per_class"  # out of "per_class" only 
    SEMANTIC_FILTERING = 0
    MODEL_CONFIDENCE_BASED_FILTERING = 0
    ...
    if DATASET == "planes":
        ...
        if PROMPT_TYPE == "txt2sentence-per_class":
            ...
            prompts_file = Path(__file__).parent.parent / "prompts_engineering/txt2sentance_prompts/LE_200_cars_all_classes_False.json"
        elif PROMPT_TYPE == "gpt-meta_class":
            prompts_file = Path("").parent / "prompts_engineering/gpt_prompts" / f"{DATASET}-100-gpt_v1.txt"
    ...
    output_folder = f"{ds_utils.root_path}/aug_data/{base_model_folder}/{CONTROLNET}/{prompt_str}_seed_{SEED}_num_inf_steps_{NUM_INFERENCE_STEPS}_gs_{GUIDANCE_SCALE}/images"
